...
void PlayerProxy::play(const string &url) {
    ...
    setOnShutdown([weakSelf, piFailedCnt](const SockException &err) {
        ...
        strongSelf->_srt_ts_src.reset();
        strongSelf->_srt_ts_ready = false;
...
void PlayerProxy::setSrtTsPassthrough() {
#if defined(ENABLE_SRT)
    auto srt_player = dynamic_pointer_cast<SrtPlayerImp>(_delegate);
    if (!srt_player) {
        return;
    }
    weak_ptr<PlayerProxy> weakSelf = shared_from_this();
    srt_player->setOnTsPacket([weakSelf](const Buffer::Ptr &packet) {
        auto strongSelf = weakSelf.lock();
        if (!strongSelf) {
            return;
        }
        strongSelf->onSrtTsPacket(packet);
    });
#endif
}

void PlayerProxy::onSrtTsPacket(const Buffer::Ptr &packet) {
    if (!packet || !packet->size()) {
        return;
    }
    if (!_srt_ts_src) {
        _srt_ts_ticker.resetTime();
        _srt_ts_ready = false;
        _srt_ts_src = std::make_shared<TSMediaSource>(_tuple, 1);
        _srt_ts_src->setListener(shared_from_this());
        setMediaSource(_srt_ts_src);
    }

    auto ts_packet = std::make_shared<TSPacket>(packet);
    ts_packet->time_stamp = _srt_ts_ticker.elapsedTime();
    _srt_ts_src->onWrite(std::move(ts_packet), true);
...
void PlayerProxy::onSrtTsProxyReady() {
    _srt_ts_ready = true;
    _timer.reset();
    _live_ticker.resetTime();
    _live_status = 0;
    _status = std::make_shared<std::string>("playing");

    _transtalion_info.byte_speed = _srt_ts_src ? _srt_ts_src->getBytesSpeed() : -1;
    _transtalion_info.start_time_stamp = _srt_ts_src ? _srt_ts_src->getCreateStamp() : 0;
    _transtalion_info.stream_info.clear();

    if (_on_play) {
        _on_play(SockException(Err_success, "srt ts passthrough play success"));
        _on_play = nullptr;
    }
    _on_connect(_transtalion_info);
    InfoL << "play " << _pull_url << " success by srt ts passthrough";
}
...
void PlayerProxy::rePlay(int iFailedCnt) {
    ...
    _timer = std::make_shared<Timer>(iDelay / 1000.0f, [weakSelf, iFailedCnt]() {
        ...
        strongPlayer->setSrtTsPassthrough();
        return false;
    }, getPoller());
}
